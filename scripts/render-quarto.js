...
function checkQuartoInstallation() {
  try {
    ...
    console.warn('To enable proper rendering, install Quarto from https://quarto.org/docs/get-started/');
...
function renderQuartoFile(filePath) {
  ...
  try {
    ...
    execSync(`cd "${fileDir}" && quarto render "${fileName}" --to html --embed-resources --standalone -o "${tempOutputName}"`, {
      stdio: 'inherit'
    });
    ...
    if (frontmatterMatch) {
      ...
      const titleMatch = /title:\s*["']?([^"'\n]+)["']?/i.exec(frontmatterBlock);
      if (titleMatch) {
        title = titleMatch[1].trim();
      }
    }
    
    // Very simple fallback HTML
    const fallbackHtml = `
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    ...
    .note { background-color: #ffffcc; padding: 15px; border-left: 5px solid #ffcc00; }
  </style>
</head>
<body>
  <h1>${title}</h1>
