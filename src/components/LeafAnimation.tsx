...
const LeafAnimation: React.FC = () => {
  ...
  const [devicePerformance] = useState<'low' | 'medium' | 'high'>(detectPerformance());
  ...
  useEffect(() => {
    ...
    const leaves: Leaf[] = [];
    const leafCount = devicePerformance === 'low' ? 75 : devicePerformance === 'medium' ? 150 : 200;
        ...
        case 3: // Simplified fern leaf
          if (devicePerformance === 'medium') {
            ...
            for (let i = 0; i < leafletCount; i++) {
              ...
              const leafletSize = halfSize * 0.7 * (i < leafletCount / 2 ? i / (leafletCount / 2) : (leafletCount - i) / (leafletCount / 2));
            ...
            for (let i = 0; i < leafletCount; i++) {
              ...
              const leafletSize = fernWidth * (i < leafletCount / 2 ? i / (leafletCount / 2) : (leafletCount - i) / (leafletCount / 2));
    ...
    const animate = (timestamp: number) => {
      ...
      if (elapsed > frameInterval || lastFrameTimeRef.current === 0) {
        ...
        const batchSize = devicePerformance === 'low' ? 
          Math.floor(leaves.length / 2) : leaves.length;
          
        // On low performance, update only a subset of leaves each frame
        const startIdx = devicePerformance === 'low' ? 
          Math.floor(Math.random() * (leaves.length - batchSize)) : 0;
          
        for (let i = startIdx; i < startIdx + batchSize && i < leaves.length; i++) {
          const leaf = leaves[i];
          ...
          const heightFactor = 1.0 + ((canvas.height - leaf.y) / canvas.height) * 0.5;
          const currentWind = baseWind * windDirection * windStrength * heightFactor;
          ...
          if (isMouseMoving && devicePerformance !== 'low') {
            ...
            if (distSquared < mouseFalloff * mouseFalloff) {
              ...
              if (dist < mouseFalloff) {
                ...
                const forceMagnitude = mouseForce * Math.pow(1 - dist / mouseFalloff, 1.5);
                ...
                leaf.rotationSpeed += forceMagnitude * 0.003 * (Math.random() - 0.5);
              }
            }
          }
          ...
          if (i % (devicePerformance === 'low' ? 3 : 1) === 0) { // Update less frequently on low-end
